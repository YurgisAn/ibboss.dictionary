...
import { MultiSelectField, MultiSelectFieldProps } from './index';
import { CheckboxField } from '~/components/form/CheckboxField';
import { INPUT_DIMENSIONS_VALUES } from '~/components/input/inputTypes';

export default {
  title: 'Example / MultiSelectField',
...
`;
const Template: Story<MultiSelectFieldProps> = (props) => {
  const items = [
        ...
        {items.map((item) => {
          return (
            <DropDownItem role="option" value={item.label} key={item.id}>
              <CheckboxField checked={selectMultiValue.indexOf(item.label) > -1}>{item.label}</CheckboxField>
            </DropDownItem>
          );
        })}
      </MultiSelectField>
