...
const Template: Story<MultiSelectProps> = (props) => {
  ...
  return (
    <MultiSelect {...props} value={selectValue} placeholder={'Placeholder'} onChange={(value) => setValue(value)}>
      {options.map((item) => {
        return (
          <DropDownItem disabled={item.disabled} role="option" value={item.label} key={item.id}>
            <CheckboxField
...
const Template2: Story<MultiSelectProps> = (props) => {
  const [selectValue, setValue] = useState<string[]>([]);
...
const Template3: Story<MultiSelectProps> = (props) => {
  const [selectValue, setValue] = useState<string[]>([]);
  ...
  return (
    <MultiSelect {...props} placeholder={'Placeholder'} value={selectValue} onChange={(value) => setValue(value)}>
      <DropDownItem onClick={() => setValue([])} onKeyDown={handleKey} tabIndex={0}>
        <WrapperItem>
...
const Template4: Story<MultiSelectProps> = (props) => {
  const [selectValue, setValue] = useState<string[]>([]);
  const options = [
