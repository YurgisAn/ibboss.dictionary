...
describe('MultiButton', () => {
  ...
  it("should pass on menu's props to OverflowMenu component", () => {
    ...
    const overflowMenu = wrapper.childAt(1);

    expect(overflowMenu.prop('list')).toEqual(menuProps.list);
    expect(overflowMenu.prop('disabled')).toEqual(menuProps.disabled);
  });

  it('should render DropdownButton with arrow down icon by default', () => {
    const wrapper = shallow(<MultiButton {...requiredProps} />);
    const dropdownButton = wrapper.childAt(0).childAt(2);
  ...
  it('should render DropdownButton with arrow up icon when user clicks on it', () => {
    ...
    const updatedButton = wrapper.childAt(0).childAt(2);
  ...
  it('should open menu when user clicks on DropdownButton', () => {
    ...
    const dropdownButton = wrapper.childAt(0).childAt(2);
    const overflowMenu = wrapper.childAt(1);
    expect(overflowMenu.prop('menuIsOpen')).toBeFalsy();

    dropdownButton.simulate('click');
    wrapper.update();

    const updatedOverflowMenu = wrapper.childAt(1);
    expect(updatedOverflowMenu.prop('menuIsOpen')).toBeTruthy();
  });
  ...
  it('should call onChange with new value when user chooses one from the list', () => {
    ...
    const overflowMenu = wrapper.childAt(1).dive();
